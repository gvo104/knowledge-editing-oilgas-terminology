...
def layer_stats(
    ...
    def load_local_dataset(ds_path: Path):
        suffix_loaders = {
            ".json": ("json", "json"),
            ".jsonl": ("json", "json"),
        ...
        def normalize_text_field(dataset):
            if "text" in dataset.column_names:
                return dataset
            first_col = dataset.column_names[0]
            return dataset.rename_column(first_col, "text")

        if ds_path.is_file():
            if ds_path.suffix not in suffix_loaders:
                raise ValueError(f"Unsupported local corpus file: {ds_path}")
            _, loader_name = suffix_loaders[ds_path.suffix]
            if loader_name == "arrow":
                dataset = Dataset.from_file(str(ds_path))
                return {"train": normalize_text_field(dataset)}
            loaded = load_dataset(loader_name, data_files=str(ds_path))
            loaded["train"] = normalize_text_field(loaded["train"])
            return loaded

        if not ds_path.is_dir():
            raise ValueError(f"Local corpus path does not exist: {ds_path}")

        files = []
        for suffix in suffix_loaders:
            files.extend(sorted(ds_path.rglob(f"*{suffix}")))
        if not files:
            raise ValueError(f"No supported corpus files found under: {ds_path}")

        grouped = {}
        for file_path in files:
            grouped.setdefault(file_path.suffix, []).append(str(file_path))

        datasets_list = []
        for suffix, file_list in grouped.items():
            _, loader_name = suffix_loaders[suffix]
            if loader_name == "arrow":
                datasets_list.extend(
                    normalize_text_field(Dataset.from_file(path)) for path in file_list
                )
                continue
            loaded = load_dataset(loader_name, data_files=file_list)
            datasets_list.append(normalize_text_field(loaded["train"]))
    ...
    def get_ds():
        ...
        if ds_path.exists():
            raw_ds = load_local_dataset(ds_path)
        else:
            raw_ds = load_dataset(
                ds_name,
    ...
    ds_name_for_path = ds_name.replace("/", "_").replace("\\", "_")
    file_extension = f"{model_name}/{ds_name_for_path}_stats/{layer_name}_{precision}_{'-'.join(sorted(to_collect))}{size_suffix}.npz"
