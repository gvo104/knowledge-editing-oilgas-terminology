...
If you have already cloned the repository and plan to run on a GPU, the shortest path is:
...
If `torch.cuda.is_available()` prints `False`, install a CUDA-enabled PyTorch build that matches your driver and CUDA stack, then re-run the check above. Most example configs in this repository already use `cuda:0`.
